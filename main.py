...
def _claude_generate_text(user_message_content: str, *, system_prompt: str) -> str:
...
def _claude_reply_and_push_sync(push_to: str, user_text: str) -> None:
    ...
    user_message = _build_claude_user_message(
        user_text, line_logs=line_logs, gmail_list=gmail_list
    )
    try:
        reply = _claude_generate_text(user_message, system_prompt=SECRETARY_SYSTEM_PROMPT)
...
def _claude_summary_and_push_sync(push_to: str) -> None:
    ...
    if user_message is None:
        ...
        return
    try:
        reply = _claude_generate_text(user_message, system_prompt=SUMMARY_SYSTEM_PROMPT)
    except APIError as e:
        reply = (
